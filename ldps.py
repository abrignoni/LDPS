...
layout = [  [sg.Text('Lab Data Pull Script.', font=("Helvetica", 25))], #added font type and font size
			[sg.Text('https://github.com/abrignoni/LDPS', font=("Helvetica", 18))],#added font type and font size
			[sg.Text('Select the file type or directory of the target extraction.', font=("Helvetica", 16))],#added font type and font size
...
while True:
	...
	logfunc('Extraction started. Please wait. This may take a few minutes...')

	
	window.refresh()
	logfunc('\n--------------------------------------------------------------------------------------')
	logfunc('LDPS: Lab Data Pull Script')
	logfunc('Objective: Extract directories and files in their paths.')
	logfunc('By: Alexis Brignoni | @AlexisBrignoni | abrignoni.com')
	window.refresh()
	
	if extracttype == 'fs':
		
		logfunc(f'Item categories to extract: {str(len(tosearch))}')
		...
		for key, val in tosearch.items():
			filefound = search(pathto, val, reportfolderbase)
			window.refresh()
			if not filefound:
				window.refresh()
				logfunc('')
				...
				for pathh in filefound:
					logfunc(f'Files for {val} located at {pathh}.<br><br>')
	...
	basep = os.getcwd()
	webbrowser.open_new_tab('file://'+basep+base+'Script Logs/Screen Output.html')
	sys.exit()
